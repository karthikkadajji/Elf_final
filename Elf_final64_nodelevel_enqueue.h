...
protected:
    ...
    int num_threads=1;
    ThreadPool *pool = new ThreadPool(this->num_threads);
        ...
        for (auto it = std::next(pointerNextDim.begin()); it != pointerNextDim.end(); ++it) {
            if (*it < LAST_ENTRY_MASK64) {
                ...
                results_get.push_back(pool->submit([=] {
                    this->partialMatch1(1, *it, resultTIDs, lowerBoundQuery, upperBoundQuery, columnsForSelect, pool);
                }));
...
//                std::unique_lock<std::mutex> lock(m_mutex);
                ...
                results_get.push_back((pool->submit([=] {this->partialMatchMonoList(1, *it, resultTIDs, lowerBoundQuery, upperBoundQuery, columnsForSelect,pool);})));
        ...
        if (pointerNextDim.front() < LAST_ENTRY_MASK64) {
            ...
            pointer &= RECOVER_MASK64;
            partialMatchMonoList(1, pointer, resultTIDs, lowerBoundQuery, upperBoundQuery, columnsForSelect, pool);

        }

        for (int i=0; i < results_get.size();i++ ) {
            while (results_get[i].wait_for(std::chrono::seconds(0)) != future_status::ready && results_get[i].wait_for(std::chrono::seconds(0)) != future_status::deferred);
        ...
        if (columnsForSelect[DIMENSION]) {
                    ...
                    if (pointer < LAST_ENTRY_MASK64) {//no monlist found
                        ...
                        results_get.push_back(pool->submit([=] {
                            this->partialMatchMonoList(DIMENSION + 1, pointer, RESULTS, lowerBoundQuery,
                                                       upperBoundQuery, columnsForSelect, pool);
                        }));
            ...
            if (isIn(lowerBoundQuery[DIMENSION], upperBoundQuery[DIMENSION], toCompare)) {
                ...
                if (pointer < LAST_ENTRY_MASK64) {//no monlist found
                    ...
                    results_get.push_back(pool->submit([=]{this->partialMatchMonoList(DIMENSION + 1, pointer, RESULTS, lowerBoundQuery, upperBoundQuery, columnsForSelect, pool);}));

                }
            }
            if (buffer != 0) {
                if (buffer < LAST_ENTRY_MASK64)
                    partialMatch1(DIMENSION + 1, buffer, RESULTS, lowerBoundQuery, upperBoundQuery, columnsForSelect, pool);
                else
            ...
            if (pointer < LAST_ENTRY_MASK64){//no monlist found
                ...
                results_get.push_back(pool->submit([=] {this->partialMatch1(DIMENSION + 1, pointer, RESULTS, lowerBoundQuery, upperBoundQuery,columnsForSelect, pool);}));
            }
    ...
    void partialMatchMonoList(uint32_t DIMENSION, uint64_t START_MONO_LIST, vector<uint32_t>* RESULTS, uint32_t* lowerBoundQuery, uint32_t* upperBoundQuery, bool* columnsForSelect, ThreadPool *pool) {
        uint32_t value;
        uint32_t i = 0;
        ...
        RESULTS->push_back(ELF[START_MONO_LIST + (NUM_DIM - DIMENSION)] & RECOVER_MASK);
