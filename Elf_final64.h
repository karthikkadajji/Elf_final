#ifndef ELF_FINAL64_H
#define ELF_FINAL64_H
...
class Elf_final64 : public Index {
protected:
    uint32_t* ELF;
    uint32_t* max_dims;
...
public:
    ...
    Elf_final64(Store* s, uint32_t* max) : Index(s), max_dims(max), NUM_DIM(s->NUM_DIM) {
        //nothing todo
    }

    Elf_final64(Store* s, Elf_final64* elf) : Index(s), ELF(elf->ELF), NUM_DIM(elf->NUM_DIM), MAX_FIRST_DIM(elf->MAX_FIRST_DIM) {
        //nothing todo
    }
...
//            ELF1 = ELF;
    ...
    vector<uint32_t>* partialMatch(uint32_t* lowerBoundQuery, uint32_t* upperBoundQuery, bool* columnsForSelect) {
        ...
        for (uint32_t offset = LOWER; offset <= UPPER; offset += 2) {
            pointerNextDim = get64(&(ELF[offset]));
            if (pointerNextDim < LAST_ENTRY_MASK64) {
                partialMatch(1, pointerNextDim, resultTIDs, lowerBoundQuery, upperBoundQuery, columnsForSelect);
            } else {
                pointerNextDim &= RECOVER_MASK64; //Unique value in First dim -> MonList found
                partialMatchMonoList(1, pointerNextDim, resultTIDs, lowerBoundQuery, upperBoundQuery, columnsForSelect);
            }
        }
    ...
    void partialMatch(uint32_t DIMENSION, uint64_t START_LIST, vector<uint32_t>* RESULTS, uint32_t* lowerBoundQuery, uint32_t* upperBoundQuery, bool* columnsForSelect) {
        uint32_t toCompare;
        uint64_t position = START_LIST;
        uint64_t pointer;
        ...
        if (columnsForSelect[DIMENSION]) {
            while ((toCompare = ELF[position]) < LAST_ENTRY_MASK) {//not found end of list, is skipped totally for 1-element list
                if (isIn(lowerBoundQuery[DIMENSION], upperBoundQuery[DIMENSION], toCompare)) {
                    pointer = get64(&(ELF[position + 1]));
                    if (pointer < LAST_ENTRY_MASK64)//no monlist found
                        partialMatch(DIMENSION + 1, pointer, RESULTS, lowerBoundQuery, upperBoundQuery, columnsForSelect);
                    else {
                        pointer &= RECOVER_MASK64;
                        partialMatchMonoList(DIMENSION + 1, pointer, RESULTS, lowerBoundQuery, upperBoundQuery, columnsForSelect);
                    }
                } else if (upperBoundQuery[DIMENSION] < toCompare) {//XXX @David scheint sinn zu machen
                    return;
                }//else continue
    ...
    inline void partialMatchMonoList(uint32_t DIMENSION, uint64_t START_MONO_LIST, vector<uint32_t>* RESULTS, uint32_t* lowerBoundQuery, uint32_t* upperBoundQuery, bool* columnsForSelect) {
        uint32_t value;
        //		uint32_t dim;
        uint32_t i = 0;
