...
void BwVColumnIterator<CODE_SIZE>::Seed(TupleId tuple_id) {
  ...
  if (code_id_in_block_ < kNumCodesPerBlock) {
    if (delta + code_id_in_segment_ < kNumCodesPerSegment) {
      ...
      group_word_id_ = (code_id_in_block_ / kNumCodesPerSegment)
          * kNumBitsPerGroup;
      last_group_word_id_ = (code_id_in_block_ / kNumCodesPerSegment)
          * kNumBitsLastGroup;
    }
  } else {
...
Status BwVColumnIterator<CODE_SIZE>::GetCode(TupleId tuple_id, Code &code) {
  ...
  size_t offset_in_segment = kNumCodesPerSegment - 1 - code_id_in_segment_;
  ...
  for (size_t group_id = 0; group_id < kNumFullGroups; group_id++) {
    ...
    for (size_t bit_id_in_group = 0; bit_id_in_group < kNumBitsPerGroup;
        bit_id_in_group++) {
      code_word |= ((data[word_id] & mask) >> offset_in_segment)
  ...
  WordUnit *data = column_block_->data_[kLastGroup];
...
Status BwVColumnIterator<CODE_SIZE>::SetCode(TupleId tuple_id, Code code) {
  ...
  for (size_t group_id = 0; group_id < kNumFullGroups; group_id++) {
    ...
    for (size_t bit_id_in_group = 0; bit_id_in_group < kNumBitsPerGroup;
        bit_id_in_group++) {
      data[word_id] &= ~mask;
      data[word_id] |= ((code_word >> (kNumBitsPerCode - 1 - bit_id))
          << offset_in_segment) & mask;
      word_id++;
      bit_id++;
    }
  }
