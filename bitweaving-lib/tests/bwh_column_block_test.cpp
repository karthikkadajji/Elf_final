...
class BwHColumnBlockTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    block_ = new BwHColumnBlock<CODE_SIZE>();
...
TEST_F(BwHColumnBlockTest, CheckBulkloadTest) {
  Code code;
  Status status;
...
TEST_F(BwHColumnBlockTest, CheckScanArgumentTest) {
  ...
  column = new BwHColumnBlock<CODE_SIZE + 1>();
  ...
  column = new BwHColumnBlock<CODE_SIZE>();
...
TEST_F(BwHColumnBlockTest, LogicalOptTest) {
...
TEST_F(BwHColumnBlockTest, FileOperationsTest) {
  Status status;
  SequentialWriteFile out_file;
  status = out_file.Open("./tests/data/test_1");
  EXPECT_TRUE(status.IsOk());
  ...
  BwHColumnBlock<CODE_SIZE> *test_block = new BwHColumnBlock<CODE_SIZE>();
  BwHColumnBlock<CODE_SIZE> *block = reinterpret_cast<BwHColumnBlock<CODE_SIZE>*>(block_);
  SequentialReadFile in_file;

  status = in_file.Open("./tests/data/test_1");
  EXPECT_TRUE(status.IsOk());
  ...
  EXPECT_EQ(0, memcmp(test_block->data_, block->data_, block->num_used_words_ * sizeof(WordUnit)));

  status = in_file.Close();
  EXPECT_TRUE(status.IsOk());

  Env env;
  status = env.DeleteFile("./tests/data/test_1");
  EXPECT_TRUE(status.IsOk());
