...
#ifndef dwarf_ImprintSIMD64WithoutCompression_h
#define dwarf_ImprintSIMD64WithoutCompression_h
...
class ImprintSIMD64IndexWithoutCompression : Index
{
private:
    ...
    uint8_t* range_select_column(const uint32_t curDim, const uint32_t lower, const uint32_t upper){
        ...
        for(curCacheLine = 0;curCacheLine<NUM_CACHE_LINES/2;curCacheLine++){
            //            cout << "huhu" << endl;
            curSIMDImprint =_mm_load_si128(&(SIMD_IMPRINT_VECTOR[curCacheLine]));
            tid = curCacheLine*32;
            
            if(!_mm_test_all_zeros (curSIMDImprint, SIMDmask)){//hit outer mask
                if (!_mm_test_all_zeros (curSIMDImprint, SIMDInnermask)){
                    SIMDcheck32Tids_AND(&(simd_array[tid/4]),tid, result, lowerSIMD, upperSIMD);
                }
            }else{
//                count++;
                result[tid/8]=0;
        ...
        //        cout << "huhu" << endl;
        for(curCacheLine*=2 ;curCacheLine<NUM_CACHE_LINES;curCacheLine++){
...
public:
    ImprintSIMD64IndexWithoutCompression(Store* s) : Index(s), NUM_CACHE_LINES((s->NUM_POINTS+15)/16), NUM_DIM(s->NUM_DIM){
    ...
    uint32_t exactMatch(uint32_t* query){
        ...
        __m128i curSIMDImprint;
        uint32_t curCacheLine;
        
        for(curCacheLine = 0;curCacheLine<NUM_CACHE_LINES/2;curCacheLine++){
            
            curSIMDImprint =_mm_load_si128(&(SIMD_IMPRINT_VECTOR[curCacheLine]));
            
            if(!_mm_test_all_zeros (curSIMDImprint, SIMDmask)){//hit mask
                for(tid = curCacheLine*32;tid<(curCacheLine+1)*32;tid++){
...
//                count++;
    ...
    ~ImprintSIMD64IndexWithoutCompression(){
        for(uint32_t dim = 0; dim < NUM_DIM; dim++){
            free ((*cols)[dim]);
            //                   delete[] result;
